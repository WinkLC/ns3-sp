...
void install_applications2(NodeContainer servers, double requestRate, struct cdf_table *cdfTable,long &flowCount, long &totalFlowSize, int SERVER_COUNT, int LEAF_COUNT, 
    ...
    while(!in.eof()){
        Flow f;
        in>>f.src>>f.dest>>f.port>>f.size>>f.start;
        flows.push_back(f);
    }
    in.close();
    ...
    for(std::size_t i=0;i<flows.size()-1;i++){
        ...
        uint32_t flowSize = flows[i].size;
        double startTime = flows[i].start;
...
int main (int argc, char *argv[])
{
    ...
    {
        NS_LOG_ERROR ("The running mode should be TLB, Conga, Conga-flow, Conga-ECMP, Presto, FlowBender, DRB , ECMP");
        return 0;
    }
...
*/
    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
                ...
                for (int k = 0; k < LEAF_COUNT; k++)
		{
		        if (k != i)
		        {
			        congaRoutingHelper.GetCongaRouting (leaves.Get (i)->GetObject<Ipv4> ())->AddRoute (leafNetworks[k],
				  	                            Ipv4Mask("255.255.255.0"),netDeviceContainer.Get (0)->GetIfIndex ());
                        } 
                }
		...
		congaSpine->AddRoute (leafNetworks[i],Ipv4Mask("255.255.255.0"),netDeviceContainer.Get (1)->GetIfIndex ());
                ...
                for (int k = 0; k < LEAF_COUNT; k++)
		{
		    if (k != i)
		    {
                ...
                drillSpine->AddRoute (leafNetworks[i],Ipv4Mask("255.255.255.0"),netDeviceContainer.Get (1)->GetIfIndex ());
	    }
